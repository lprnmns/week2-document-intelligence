...
Kaynak: `Alperen_Manas_Yapay_Zeka_Muhendisligi_Gelisim_Programi_2_Hafta.pdf`
...
| Versioning, staged inactive points, verification and atomic activation | §10, p. 10 | PASS (MVP scope) | `IngestionWorker`, `QdrantChunkStore`, ADR-003; activation tests; final 26-point ingest | Retention/purge intentionally remains bounded to the current MVP; no expanded purge feature is claimed |
...
| Payload metadata: document/version/active/tenant/ACL/page/source/pipeline/model identity | §§11–12, pp. 11–12 | PASS (final demo corpus) | `eval/verify_active_metadata.py`; `eval/results/final_corpus_metadata.json`; 26/26 active points inspected, missing required metadata `0` | The preserved 135-point legacy collection is not the final demo corpus and is not used for the final evaluation claim |
...
| Reproducible run manifest: SHA, dataset, corpus snapshot, models, config, host | §16, p. 16 | PASS (local demo) | `eval/reporting.py`, `week2_stabilization_v1/run_manifest.json`, `/v1/evaluations/config`, live run IDs and `DIS_SOURCE_REVISION` injection | Container direct-start without the provenance variable reports `unknown`; final release requires a committed/rebuilt tree |
...
| CI: lint, type, unit/contract/security/evaluation/image checks | §22, p. 22 | PASS | `.github/workflows/document-intelligence-service.yml`, `pyproject.toml` | Yeni regression testlerinin CI’de kaldığını doğrula |
| Five-day gates and evaluation/idempotency priority | §23, p. 23 | PASS | `docs/internship/week2/hafta2_uygulama_plani.md` | Final report’ta açık gap’leri saklama |
...
| Technical interview questions with decision/evidence/alternative/limitation | §27, p. 27 | PASS | `docs/internship/week2/mentor_teknik_sorular_cevaplari.md`, ADRs | New ingestion/model decisions are documented below |
| `.env.example`, at least five ADRs, official references, final SHA note | §28, p. 28 | PASS | `.env.example`; ADR-001..008; README/docs; manifests | ADR-008 numbering note is intentional because ADR-006/007 already exist |
