...
| 14 | BM25/dense/hybrid query slices; Recall/MRR/nDCG; p50/p95; 5 gain/5 loss | `benchmark_report.md`, `eval/reporting.py`, raw CSV/JSONL, flip analysis | PASS | 2026-08-10 clean snapshot: 44 vaka, 26 active point; sonuçlar bu snapshot ile sınırlı |
...
| 17 | 40+ dengeli golden set; evidence labels; adjudication; validation-only threshold; test leakage yok | `data/evaluations/mentor_program_pdf_rag_golden_v1.jsonl` (44), split tests, calibration report | PASS | Mentorun kör `%20` incelemesi repo içinde yalnız hazırlık notu; dış review bekleniyor |
...
| 21 | Compose diagramı: API, worker, Qdrant, UI, host Ollama, volume, healthcheck, limits | `compose.yaml`, `.env.example`, `run_document_service_compose_smoke.sh` | PASS | Ollama bağlantısı makine/network'e bağlı; smoke bunu açıkça kontrol ediyor |
...
| 23 | 5 günlük gate planı ve “evaluation/idempotency azaltılmaz” kuralı | `docs/internship/week2/hafta2_uygulama_plani.md`, gün gate'leri | PASS | Takvim planı ile gerçek commit tarihleri ayrıca raporlanmalı |
...
| 27 | 12 teknik görüşme sorusunu ölçüm/kod/alternatif/sınır ile cevaplama | `docs/internship/week2/mentor_teknik_sorular_cevaplari.md` | PASS | Cevaplar mevcut commit SHA'sındaki artifact'lere bağlı |
